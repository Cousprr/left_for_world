...
在Lab2下新建一个文件夹存放代码，使用`git init`初始化仓库。  
...
在文件`/etc/apache2/apache2.conf`中找到`<Directory /var/www/>`，复制一个directory，修改路径为`/home/dbms/Lab2/your_dir/`，其他设置不动。

在`/etc/apache2/sites-available/000-default.conf`中找到`DocumentRoot /var/www/html`，修改为`DocumentRoot /home/dbms/Lab2/your_dir`。

保存，重启apache服务`sudo service apache2 restart`。这时再输入localhost导向的就是我们自己写的index.php。

如果出现 forbidden 错误，先确认上面两个文件修改正确并保存了，如果没问题就是apache没有这个仓库权限。

```
#先查看apache用户名，一般是www-data
ps aux | grep apache
#修改权限
sudo chown -R www-data:www-data /home/dbms/Lab2/your_dir
sudo chmod -R 755 /home/dbms/Lab2/your_dir
#重启apache
...
实验要求制作一个火车订票系统并实现9个功能，包括查询、订票等功能，总体仿照12306。